...
- Sound is judged by the same yardstick. A burst of sound shorter than the minimum silence run, separated from the utterance by silence, is an artifact rather than speech, and is stripped along with the silence around it. Without this rule a single click at the end of a file hides all the silence in front of it: the `él` fixture ends with eleven stray samples in its final 3.1 ms, which was enough to make the tool report the whole file as unchanged.
