...
**What to build:** A way to preview a run before committing to it, which matters most before a destructive `--in-place` pass. `--dry-run` performs the full analysis and produces exactly the report a real run would, but writes nothing to disk — no destinations, no temporary files, no in-place replacements.
...
- [x] `--dry-run` works alongside `--in-place` and `-o`, reporting what each would have done
...
**Nothing is written in any destination mode** — default, `--in-place` and `-o` are each covered, asserting both the directory listing and the source's mtime are untouched. Verified to fail if the preview path is removed.

**Review fix: a dry run reported success for runs certain to fail.** Because the cut goes to the system temporary directory, no destination write was ever attempted, so an unwritable destination — the exact thing a preview is for, before a destructive `--in-place` pass — came back clean at exit 0 while the real run exited 1. `check_writable` now confirms the destination's directory exists and is writable, without creating anything. The only exit-code test covered a corrupt *source*, which agreed by luck.

**Added `-n` as a short form for `--dry-run`.** It is the conventional spelling in `make`, `rsync` and `git clean`, and it is safe in a way `-i` is not: the worst a mistyped `-n` can do is decline to write. Both spellings share one `dry_run` destination, and `test_n_is_short_for_dry_run` asserts the two produce identical output and neither writes a destination.
