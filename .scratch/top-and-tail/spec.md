...
Vocabulary audio generated by text-to-speech arrives heavily padded with silence. Across the example sources, the utterance occupies only 20–39% of the file; the rest is leading and trailing silence:
...
`él` adds a second trap: its trailing silence does not reach the end of the file. Eleven stray samples in its final 3.1 ms sit above the threshold, so anything that looks for silence only at the very edge of a file finds none at all.
...
- Sound is judged by the same yardstick. A stretch of sound shorter than the minimum silence run, cut off from the rest by silence, is an artifact rather than part of the utterance, and is stripped along with the silence around it. Any number of artifacts can be shed from either end in a single pass, and a source that is nothing but artifacts has no utterance and is left unchanged. Without this rule a single click at the end of a file hides all the silence in front of it: the `él` fixture ends with eleven stray samples in its final 3.1 ms, which was enough to make the tool report the whole file as unchanged.
...
Four examples are committed to the repository as fixtures, and none is incidental. `pretérito` is the only thing standing between the project and a regression that eats internal silence. `él` is the only one whose trailing silence is followed by an artifact, and it was added after that case was found to defeat detection entirely.
